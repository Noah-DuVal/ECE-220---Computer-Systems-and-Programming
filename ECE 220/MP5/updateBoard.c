...
int countLiveNeighbor(int* board, int boardRowSize, int boardColSize, int row, int col){
    ...
    for (i = row-1; i <= row + 1; i++) {        //circulates through the rows
        for (j = col-1; j <= col + 1; j++) {    //circulates through the cols
            if (i != row || j != col) {         //checks that its not the middle piece
                if (i < 0 || i > boardRowSize - 1 || j < 0 || j > boardColSize - 1 ) {   //cell is out of bounds and do not add to count
                    count += 0;
...
void updateBoard(int* board, int boardRowSize, int boardColSize) {
    ...
    int k, m;
    int q, w;
    int boardcopy[boardColSize*boardRowSize];       //sets up copy array to size of board
    
    //copies the the game board into board copy by going through each cell individualy and adding it to an empty array
    ...
    //the following goes through each cell and uses the # of live neighbors to determine whether the cell lives or dies
    
    for (k = 0; k <= boardRowSize - 1; k++) {
        for (m = 0; m <= boardColSize - 1; m++) {
            if (boardcopy[ k * boardColSize + m ] == 1) { // Cell is alive
                if (countLiveNeighbor(boardcopy, boardRowSize, boardColSize, k, m) == 2) {  //if 2 live neighbors it lives on
                    board[ k * boardColSize + m ] = 1; // cell lives on
		}
                else if (countLiveNeighbor(boardcopy, boardRowSize, boardColSize, k, m) == 3) {     //if 3 live neighbors it lives on
                    board[ k * boardColSize + m ] = 1; // cell lives on
                } 
		else {
                    board[ k * boardColSize + m ] = 0; //cell dies from under or over population if less than 2 or greater than 3 live neighbors
                }
             }
            else if (boardcopy[ k * boardColSize + m ] == 0) { // cell is dead
                if (countLiveNeighbor(boardcopy, boardRowSize, boardColSize, k, m) == 3) {  //if cell has exactly 3 live neighbors it is revived
                    board[ k * boardColSize + m ] = 1;   //cell is revived by repopulation
                    }
                }
...
int aliveStable(int* board, int boardRowSize, int boardColSize){
    ...
    int alivecopy[boardColSize*boardRowSize]; //sets up copy array to size of board
    
//makes copy of game board

    for (a = 0; a <= boardRowSize - 1; a++) {
        for (s = 0; s <= boardColSize - 1; s++) {
            alivecopy[ a * boardColSize + s ] = board[ a * boardColSize + s ];
        }
    }

    
    updateBoard(alivecopy, boardRowSize, boardColSize);		//stores updated copy in alivecopy
    
    for (c = 0; c <= boardRowSize - 1; c++) {
        for (v = 0; v <= boardColSize - 1; v++) {
            if (alivecopy[ c * boardColSize + v ] != board[ c * boardColSize + v ]) {   //if the cells are not the same then return 0
                return 0;
            }
                
        }
    }
    return 1;   //if all the cells are checked and does not return 0 then it must be the same and therefore return 1
