...
void updateBoard(int* board, int boardRowSize, int boardColSize) {
    ...
    int k, m;
    int q, w;
    int boardcopy[boardColSize*boardRowSize];
    ...
    for (k = 0; k <= boardRowSize - 1; k++) {
        for (m = 0; m <= boardColSize - 1; m++) {
            if (boardcopy[ k * boardColSize + m ] == 1) { // Cell is alive
                if (countLiveNeighbor(boardcopy, boardRowSize, boardColSize, k, m) == 2) {
                    board[ k * boardColSize + m ] = 1; // cell lives on
		}
                else if (countLiveNeighbor(boardcopy, boardRowSize, boardColSize, k, m) == 3) {
                    board[ k * boardColSize + m ] = 1; // cell lives on
                } 
		else {
                ...
                if (countLiveNeighbor(boardcopy, boardRowSize, boardColSize, k, m) == 3) {
...
int aliveStable(int* board, int boardRowSize, int boardColSize){
    ...
    int alivecopy[boardColSize*boardRowSize];
...
//makes copy of game board
    ...
    for (c = 0; c <= boardRowSize - 1; c++) {
        for (v = 0; v <= boardColSize - 1; v++) {
            if (alivecopy[ c * boardColSize + v ] != board[ c * boardColSize + v ]) {
                return 0;
            }
