...
In this mp the goal was to create a few funcitons that helped partitioning and 
planning out a floorplan for a circut design. Some of the funcitons we had to 
create included swapping the modules at the ends fo the tree, swapping the 
cutline of the internal nodes, checking what type of node it is (internal/leave)
, as well as buidling the tree, checking if it is in a certain subtree, 
and to rotate the design, and lastly to get the expression in postfix and 
calculating the total resources needed. 
...
int is_leaf_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
    ...
    if (ptr == NULL) {	//Checks that it is not root
        return 0;
    }
    
    if (ptr->module != NULL) {	//if module is null then it is cutline if not its a leave
        return 1;
    } else {
        return 0;
    }
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
    ...
    if (ptr == NULL) {	//checks its not a root
        return 0;
    }
    if (ptr->module == NULL) {	//if mod is null then its cutline/internal
        return 1;
    } else {
        return 0;
    }
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
    ...
    if (b == NULL) return 0;	//checks that its not a root
    
    if (a == b) {	// checks if a matches b
        return 1;
    }
    return is_in_subtree(a,b->parent);	//Recurses b up till it == a or is root
}
...
void rotate(node_t* ptr) {
  // TODO: 
    ...
    temp2 = ptr->module->w;		//stores w
    ptr->module->w = ptr->module->h;	//swaps h to w
    ptr->module->h = temp2;		//stores w in h
...
void recut(node_t* ptr) {
  ...
  // TODO:
    ...
    if (ptr->cutline == V) {	//if it is v then swap h
        ptr->cutline = H;
	return;
    }
    if (ptr->cutline == H) {	//if it is h then swap v
        ptr->cutline = V;
	return;
    }
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
    ...
    temp = a->module;		//store a
    a->module = b->module;	//store b in a
    b->module = temp;		//store a in b
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO:
    ...
    if (ptr->module == NULL) { //is a cutline, V or H
        postfix_traversal(ptr->left,nth, expression);	//recurse left
        postfix_traversal(ptr->right,nth, expression);	//recurse right
        expression[*nth].cutline = ptr->cutline;	//store cutline
        expression[*nth].module = NULL;			//store mod - null
	(*nth)++;					//increment
    }
    		//does center/ptr
    if (ptr->cutline == UNDEFINED_CUTLINE) { 		//is a mod, 1,2,3
        expression[*nth].module = ptr->module;		//stores mod
        expression[*nth].cutline = UNDEFINED_CUTLINE;	//store cutline - undefinded
	(*nth)++; 					//increment
    }
...
int get_total_resource(node_t* ptr)
{
  // TODO:
    ...
    for (i=0; i < num_modules-1; i++) {
        tr += ptr->right->module->resource;
        ptr = ptr->left;
    }

    tr += ptr->right->module->resource;
    tr += ptr->left->module->resource;
...
*/
   for (i=0; i < num_modules; i++) {	//loops through mod array adding values

	tr += modules[i].resource;
/*
    if (i == num_modules-1) {
        tr += ptr->right->module->resource;
        tr += ptr->left->module->resource;
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // TODO:
    ...
    if (n == num_modules-1 ) {	//last two mods at bottom tree
        //creates new node
        node_t *new_in = (node_t*)malloc(sizeof(node_t));	//allocates the mem
        new_in->module = &modules[n];		//stores mod
        new_in->left = new_in->right = NULL;	//creates the last r & l 
        new_in->parent = par;			//attatches it to the tree
        new_in->cutline = UNDEFINED_CUTLINE;	//sets the cutline
        return new_in;				//returns the new node
    }
    
    if (n < num_modules-1 ) {	//adds cutline and right and left nodes
        //creates new node
        node_t *new_in = (node_t*)malloc(sizeof(node_t));	//allocates the mem
        new_in->cutline = V;	//sets cutline
        new_in->right = (node_t*)malloc(sizeof(node_t));	//allocates mem
        new_in->right->right = new_in->right->left = NULL;	//creates r & l nodes
        new_in->right->cutline = UNDEFINED_CUTLINE;	//follwoing sets right info
        new_in->right->module = &modules[n];
        new_in->right->parent = new_in;
        new_in->left = init_slicing_tree(new_in, n+1);	//recurses to set left node
        new_in->parent = par;	//sets parent node attatching it to the tree
        new_in->module = NULL;	//sets mod
        return new_in;
    }
